...
import (
	...
	"github.com/lacsar712/cokeoven/internal/battery"
	"github.com/lacsar712/cokeoven/internal/ovenbank"
	"github.com/lacsar712/cokeoven/internal/fsm"
...
type App struct {
	...
	plant        *ovenbank.BatteryPlant
	towerFSM     *fsm.TowerFSM
	zones        *ovenbank.ZoneTable
	...
	holdMgr      *cokegas.HoldWindowManager
	holdEval     *cokegas.HoldWindowEvaluator
	gradAudit    *cokegas.OrderedGradientValidator
	guard        *interlock.Guard
	dryFSM       *fsm.CokingFSM
	scheduler    *clock.SegmentScheduler
	avgWindow    *clock.CarbonWindow
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := ovenbank.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := ovenbank.NewBatteryPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(cokegas.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := battery.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := cokegas.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    cokegas.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.battery = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	if pc, ok := a.clk.(*clock.ProcessClock); ok {
		...
		a.avgWindow = clock.NewCarbonWindow(a.clk, time.Duration(cfg.CarbonWindowMinutes)*time.Minute)
	}
	a.dryFSM = fsm.NewCokingFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewCokeRamp(a.clk, cfg.ProcessTick(), cfg.CokeRampSteps)
