...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.battery.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.battery.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.battery.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *cokegas.ProfileManager, windows *cokegas.HoldWindowManager) *cokegas.HoldWindowEvaluator {
	return cokegas.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *battery.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *battery.Controller {
	c := battery.NewController(bank, maxDelta)
